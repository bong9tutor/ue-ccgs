...
### TD-015 — Input MANUAL 체크리스트 완료 (Story 1-11 에셋 후)

| 필드 | 값 |
|---|---|
| **Severity** | S3 |
| **Source** | Story 1-21 (2026-04-19) |
| **Origin Story** | `production/epics/foundation-input-abstraction/story-005-no-raw-key-mouse-only-hover-gamepad-silent.md` |
| **File** | `production/qa/evidence/input-manual-checklist-placeholder-2026-04-19.md` |
| **Description** | Story 1-21의 MOUSE_ONLY_COMPLETENESS / HOVER_ONLY_PROHIBITION / GAMEPAD_DISCONNECT_SILENT는 MANUAL 실기 검증 필요. Story 1-11 (에셋) + Card Hand UI epic 완료 후 PIE 세션 수동 수행. |
| **Impact** | Sprint 1 완료 게이트 부분. CI hook 2개는 완성되어 회귀 방지는 즉시 가능. MANUAL 항목은 release-gate에서 확인. |
| **Proposed Fix** | Story 1-11 에셋 생성 + Card Hand UI epic 완성 후 PIE 세션 수동 수행. placeholder 문서의 Results 섹션 채우기. |
| **Owner** | 사용자 (PIE 세션) |
| **Status** | Open (선행 조건 해결 후) |
...
|---|---|
| **Severity** | S3 |
| **Source** | Story 1-20 (2026-04-19) |
| **Origin Story** | `production/epics/foundation-save-load/story-006-narrative-atomic-compound-negative.md` AC COMPOUND_EVENT_NO_SEQUENCE_ATOMICITY |
| **Description** | Story 1-20은 Save/Load에 sequence atomicity API 부재만 검증. Day 13 실제 fault injection (ECardOffered → EDreamReceived fail → ENarrativeEmitted) 통합 테스트는 GSM epic의 BeginCompoundEvent/EndCompoundEvent 구현 후 수행 필요. |
| **Impact** | Compound event sequence atomicity가 의도대로 per-trigger만 보장되는지 런타임 검증 부재. 설계 의도는 의도적 (ADR-0009)이지만 회귀 발견이 GSM epic 완성 이후까지 지연. |
| **Proposed Fix** | GSM epic (`core-game-state-machine`) 착수 시 integration test 추가: Day 13 scenario + worker thread fault injection + disk verification. Evidence: `production/qa/evidence/compound-event-negative-[date].md`. |
| **Owner** | GSM epic 담당자 |
| **Status** | Open (GSM epic 진입 시) |

### TD-013 — Pipeline Fatal threshold Shipping 강제 (UE_LOG Fatal → checkf)

| 필드 | 값 |
|---|---|
| **Severity** | S4 |
| **Source** | Story 1-19 (2026-04-19) |
| **Origin Story** | `production/epics/foundation-data-pipeline/story-006-tinit-budget-overflow-thresholds.md` |
| **File** | `MadeByClaudeCode/Source/MadeByClaudeCode/Data/DataPipelineSubsystem.cpp` (EvaluateTInitBudget, CheckCatalogSize) |
| **Description** | Story 1-19는 Fatal threshold를 `UE_LOG Error` + `[FATAL_THRESHOLD]` 접두어 + `bInitFatalTriggered` flag로 구현 (test harness crash 방지). Shipping 빌드에서 실제 process 중단을 강제하려면 `checkf(false, ...)`로 교체 필요. |
| **Impact** | 현재 Fatal 임계 초과 시 process가 계속 실행됨 (flag만 설정). Production에서 시각적 인지 없음. |
| **Proposed Fix** | `#if UE_BUILD_SHIPPING` 가드로 분기: Shipping에서는 `checkf(false, ...)`로 강제 중단, Development/Editor에서는 로그 + flag. |
| **Owner** | Data Pipeline epic 담당자 |
| **Status** | Open |

### TD-012 — Time Tick pause/focus-loss 지속 검증 (GameWorld + SetGamePaused 필요)

| 필드 | 값 |
|---|---|
| **Severity** | S4 |
| **Source** | Story 1-18 (2026-04-19) |
| **Origin Story** | `production/epics/foundation-time-session/story-005-in-session-tick-rules.md` |
| **File** | `Tests/TimeSessionTickRulesTests.cpp` (integration 분리) |
| **Description** | FTSTicker가 engine-level에서 pause/focus loss에 무관하게 tick된다는 계약은 단위 테스트에서 재현 불가 (실제 GameWorld + SetGamePaused 필요). 현재 Unit에서는 TickInSession 로직만 검증. |
| **Impact** | FTSTicker 계약 회귀가 실기에서만 발현. 만약 미래 버전에서 FTSTicker가 pause 영향 받으면 unit pass / runtime fail. |
| **Proposed Fix** | Integration test 추가: UWorld 생성 + SetGamePaused(true) 5초 → TickInSession 호출 카운트 ≥ 4회. WM_ACTIVATE loss simulation 시 3초 → ≥ 2회. |
| **Owner** | 통합 테스트 담당 |
| **Status** | Open |

### TD-011 — Input Offer Hold Formula 2 boundary 실측 (Story 1-11 에셋 + PIE 필요)

| 필드 | 값 |
|---|---|
| **Severity** | S3 |
| **Source** | Story 1-17 (2026-04-19) |
| **Origin Story** | `production/epics/foundation-input-abstraction/story-004-offer-hold-formula-thresholds.md` |
| **File** | `Tests/InputOfferHoldTests.cpp` (integration 분리) |
| **Description** | Story 1-17은 ApplySettingsToMappingContext 로직만 unit level 검증. 실제 Hold boundary (Mouse 0.15s / Gamepad 0.5s inclusive `>=`)는 UInputTriggerHold 에셋 + UEnhancedInputComponent 런타임 발화 경로 필요. Unit test에서는 Mock으로 HoldTimeThreshold 적용 검증만 가능. |
| **Impact** | Formula 2 경계값 회귀가 런타임에서만 발현. Story 1-11 에셋 + PIE 세션으로만 재현 가능. |
| **Proposed Fix** | Story 1-11 에셋 생성 후 (TD-008) Integration test 추가: OFFER_HOLD_BOUNDARY_GAMEPAD, OFFER_HOLD_CANCEL_GAMEPAD, OFFER_DRAG_THRESHOLD_MOUSE, OFFER_CLICK_SELECT_MOUSE. evidence doc 경로: `production/qa/evidence/offer-hold-boundary-*-evidence.md` |
| **Owner** | 사용자 (PIE 세션) 또는 통합 테스트 담당 |
